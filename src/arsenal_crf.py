...
def df2crfsuite(df, delim='##END'):
    ...
    # ##END also feed on model in order to compelete the text
    # result = [i for i in sents if i != []]
    return result, index
...
def token_text(test_df, y_pred, index):
    ...
    phrase = []
    sentence = []
    for i, word_tag in enumerate(crf_results):
    ...
    pd.DataFrame(sentence).to_csv(PREDICT_FILE, header=False, index=False)
