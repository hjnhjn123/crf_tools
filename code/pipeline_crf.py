...
def batch_add_features_(data, feature_sets, feature_dics):
    # temp = [map_set_2_matrix(chunk, f_set) for chunk in data for f_set in feature_sets]
    temp = []
    for f_set in feature_sets:
        temp.append([map_set_2_matrix(chunk, f_set) for chunk in data])

    temp = zip([item[-1] for ll in temp[1:] for line in ll for item in line])

    result = [map_dict_2_matrix(chunk, f_dic) for chunk in temp for f_dic in feature_dics]
    return result
